<?php
	...
	if(!empty($info->mese) && !empty($info->anno)){        // se ci sono i campi, fa la somma degli importi dove il mese e l'anno coincidono con quelli inseriti
		...
		$sql = "SELECT SUM(valore) AS risultato FROM operazione WHERE MONTH(data) = '$info->mese' AND YEAR(data) = '$info->anno'"; 
		$ris = $db->query($sql);
		if($ris -> num_rows >0){
			$ris = $ris -> fetch_assoc();
			...
			$sql_2 = "SELECT * FROM operazione WHERE MONTH(data) = '$info->mese' AND YEAR(data) = '$info->anno'"; 
