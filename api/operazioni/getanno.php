<?php
	...
	if(!empty($info->anno)){        // se ci sono i campi, fa la somma degli importi dove il mese e l'anno coincidono con quelli inseriti
		...
		$sql = "SELECT SUM(valore) AS risultato FROM operazione WHERE YEAR(data) = '$info->anno' AND nome_fornitore IS NOT NULL"; 
		$ris = $db->query($sql);
		if($ris -> num_rows >0){
			$ris = $ris -> fetch_assoc();
			...
			$sql_2 = "SELECT * FROM operazione WHERE YEAR(data) = '$info->anno' AND nome_fornitore IS NOT NULL"; 
			...
			$r = array("success"=>true,"ORDINI" => $ordine , "INCASSI" =>"Pagamenti annuali: $ris[risultato] ");
			echo json_encode($r);	
